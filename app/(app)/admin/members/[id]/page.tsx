      ...
      {ms ? (
        <MembershipAdminCard
          membership={{
            ...
            track: ms.track,
            status: ms.status,
            admission: ms.admission,
            seatedAt: ms.seatedAt?.toISOString() ?? null,
            ratifiedAt: ms.ratifiedAt?.toISOString() ?? null,
          }}
        />
      ) : (
        <p className="text-muted mt-2 text-sm">No membership (candidate).</p>
      )}
