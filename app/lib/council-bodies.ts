...
export type WgClass = "founding_member" | "observer";

/** The set of membership tracks a user holds via an *active* membership. */
export async function userActiveClasses(userId: string): Promise<Set<WgClass>> {
  ...
  for (const link of links) {
    ...
    if (m && m.status === "active") set.add(m.track as WgClass);
  }
  return set;
}

/**
 * Body/committee access rule: `any` = any active membership (Founding Members
 * and Observers — observers attend and speak); `associate` (legacy enum name
 * from the port) = Founding Members only.
 */
...
export function lockReason(requiredClass: "any" | "associate"): string {
  return requiredClass === "associate"
    ...
    : "Requires an active membership (Founding Member or Observer).";
}
