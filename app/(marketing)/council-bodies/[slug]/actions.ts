...
const inviteMessage =
  "Invitation sent — they've been asked to join the Council and will " +
  "enter the body as soon as their membership is active.";

/** Record a pending invite and email the person to join the Council. */
async function createInvite(
  ...
  notify(
    sendWgInviteEmail({
      ...
      invitedByName: inviter ? personName(inviter) : "The Verana Council",
    }),
  );
  return {
    ok: true,
...
export async function addLead(
  ...
  if (!target) {
    ...
    // Council and becomes a lead once their membership is active.
 ...
 * directly; everyone else gets a pending invite + a join-the-Council email. */
...
/** Re-send the join-the-Council email for a pending invite. */
