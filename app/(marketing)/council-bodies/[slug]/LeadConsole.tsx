    ...
    <div className="mt-6 grid gap-12 lg:grid-cols-2">
      ...
      <div>
        ...
        <p className="text-xs text-muted mt-2">
          ...
          Council and become a lead once their membership is active.
        </p>
        ...
        <p className="text-xs text-muted mt-2">
          Council Members and Observers join directly; anyone else is invited
          to join the Council and enters the body once their membership is
          active.
        </p>
        {inviteState.error && (
        ...
        {invites.length > 0 && (
          <>
            ...
            <p className="text-sm text-muted mt-1">
              Invited by email; they join the body automatically once their
              Council membership is active.
            </p>
            <ul className="mt-3 space-y-2">
              {invites.map((i) => (
                <li key={i.id} className="flex items-center gap-2 text-sm">
                  <span className="flex-1 min-w-0 truncate">{i.email}</span>
                  <span className={`badge ${i.role === "lead" ? "badge-indigo" : ""}`}>
                    {i.role}
                  </span>
                  <button
                    type="button"
