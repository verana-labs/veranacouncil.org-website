...
/**
 * Convert pending council-body invites into real lead/participant rows for
 * the given emails — the single conversion path, called from every place a
 * person can start satisfying the gate: sign-in, ballot seatings, seed
 * designations, observer acceptance (listing), admin membership-status
 * changes (incl. reinstatement), and org access-list additions.
 *
 * The gate is strict by design: an invite converts only when a user with that
 * verified email exists AND holds an active membership satisfying the body's
 * requiredClass. Idempotent and best-effort — callers (ballot settlement,
